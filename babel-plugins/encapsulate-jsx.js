    ...
    return {
        visitor: {
            ...
            JSXOpeningElement: function transform(path, state) {
                ...
                let className = classnameFromFilename(filename);

                if (className === 'index' || className === 'render') {
                    // use the parent folder's name instead
                    const splitPath = state.file.log.filename.split('/');
                    className = splitPath[splitPath.length - 2];
                }
